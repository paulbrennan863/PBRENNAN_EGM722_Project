...
#Iterate over each <Equipment> element in the AEMP XML File
for equipment in root.findall('Equipment'):
    ...
    # Append data to the list
    data.append({'OEMName': OEMname, 'Model': model, 'SerialNumber': serialNumber, 'Latitude': latitude, 'Longitude': longitude,'Hour': hour,'FuelConsumed': fuelconsumed, 'Fuel Unit': fuelunits, 'C02 Produced Kg': Co2_produced})
...
#ni_outline=gpd.read_file('Datafiles/NI_Outline.shp')
#ni_outline.to_crs(crs='epsg:4326',inplace=True)
...
m = bedrock.explore('RCS_D', m=m, cmap='viridis', opacity=0.01)
m.save('map2.html')
m.show_in_browser()
