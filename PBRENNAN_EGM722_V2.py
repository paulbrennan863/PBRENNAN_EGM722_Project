...
def xml_pasrse(filename):
    ...
    for equipment in root.findall('Equipment'):
        ...
        Co2_produced = ((float(fuelconsumed) * 2.68)/1000) #Each litre of Diesel produces 2.68kg  of C02 during combusion then divide by 1000 to get co2 tonnes

        # Append data to the list
        machine_data.append({'OEMName': OEMname, 'Model': model, 'SerialNumber': serialNumber, 'Latitude': latitude, 'Longitude': longitude,'Hour': hour,'FuelConsumed': fuelconsumed, 'Fuel Unit': fuelunits, 'C02 Produced Kg': ("%.2f"%Co2_produced)})
...
def load_bedrock_shapefile():
    ...
    machine_positions = gpd.GeoDataFrame(df[['SerialNumber', 'Model', 'Hour', 'FuelConsumed', 'C02 Produced (tonnes))']], # use the csv data, but only the name/website columns
